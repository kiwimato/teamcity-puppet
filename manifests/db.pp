...
class teamcity::db (
  $manage_repos         = false,
  $username             = 'teamcity',
  $password             = 'supersecret0067',
  $host                 = 'localhost',
  $port                 = 5432,
  $db_name              = 'teamcity_server',
  $mysql_connector      = 'mysql-connector-java-5.1.38',
  $postgresql_connector = 'postgresql-42.2.9.jar',
) {
...
",
    ...
    'mysql': {
      ...
      exec { "download-${mysql_connector}":
        command => "curl http://ftp.jaist.ac.jp/pub/mysql/Downloads/Connector-J/${mysql_connector}.tar.gz | tar -xvz   $
          {mysql_connector}/${mysql_connector}-bin.jar",
        path    => ['/bin', '/sbin', '/usr/bin'],
        cwd     => '/tmp',
